...
export function Navbar({ mounted, user }: { mounted: boolean; user: SessionUser | null }) {
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            ...
            <Link href="/" className="hidden rounded-full border border-white/10 px-4 py-2 text-sm font-semibold text-white/75 transition hover:border-white/30 hover:text-white md:block light-mode:border-black/10 light-mode:text-slate-600 light-mode:hover:border-black/20 light-mode:hover:text-slate-900">Login</Link>
          )}
      ...
      {mobileOpen ? (
        <div className="border-t border-white/10 px-4 py-4 md:hidden light-mode:border-black/10">
          <div className="flex flex-col gap-2">
            ...
            {user ? <button onClick={handleLogout} className="rounded-2xl border border-white/10 px-4 py-3 text-left text-sm font-semibold text-white/75 light-mode:border-black/10 light-mode:text-slate-700">Logout</button> : null}
          </div>
        </div>
      ) : null}
    </header>
