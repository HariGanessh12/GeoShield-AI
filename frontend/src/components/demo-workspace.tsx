      ...
      <aside className="border-b border-white/10 bg-[#0b1020] p-5 lg:border-b-0 lg:border-r lg:border-white/10 light-mode:border-slate-200 light-mode:bg-white">
          ...
          {demoRegions.map((region) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="font-bold text-white light-mode:text-slate-950">{region.name}</span>
                <span className="text-sm font-black" style={{ color: region.color }}>
                  {region.score}/100
                </span>
              </div>
              <p className="mt-2 text-sm leading-6 text-white/60 light-mode:text-slate-600">{region.summary}</p>
            </button>
          ))}
        </div>
        ...
        <div className="mt-5 rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5 light-mode:border-slate-200 light-mode:bg-slate-50">
          ...
          <div className="mt-4 space-y-3">
            {activeAlerts.map((alert) => (
              <article key={alert.title} className="rounded-2xl border border-white/10 bg-black/20 p-3 light-mode:border-slate-200 light-mode:bg-white">
                <div className="flex items-center justify-between gap-3">
                  <h3 className="text-sm font-bold text-white light-mode:text-slate-950">{alert.title}</h3>
                  <span className="rounded-full border border-white/10 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-white/50 light-mode:border-slate-200 light-mode:text-slate-500">{alert.severity}</span>
                </div>
                <p className="mt-2 text-sm leading-6 text-white/60 light-mode:text-slate-600">{alert.detail}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="mt-5 rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5 light-mode:border-slate-200 light-mode:bg-slate-50">
          ...
          <div className="mt-4 space-y-3">
            {demoRegions.map((region) => (
              <div key={region.id}>
                <div className="mb-1 flex items-center justify-between text-xs text-white/60 light-mode:text-slate-500">
                  <span>{region.name}</span>
                  <span>{region.score}</span>
                </div>
                <div className="h-2 rounded-full bg-white/10 light-mode:bg-slate-200">
        ...
        <div className="mt-5 rounded-[1.8rem] border border-cyan-500/20 bg-cyan-500/10 p-5 light-mode:border-sky-200 light-mode:bg-sky-50">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-200 light-mode:text-cyan-700">AI analysis</p>
          <h2 className="mt-2 text-lg font-black text-white light-mode:text-slate-950">{analysisStatus}</h2>
          <p className="mt-3 text-sm leading-7 text-white/72 light-mode:text-slate-600">{analysis}</p>
        </div>

        <button
          onClick={handleReportDownload}
          disabled={generatingReport}
          className="mt-5 w-full rounded-full bg-white px-5 py-3.5 text-sm font-black text-slate-900 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60 light-mode:bg-slate-950 light-mode:text-white"
        >
