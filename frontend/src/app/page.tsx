    ...
    <div className="flex min-h-[calc(100vh-81px)] items-center justify-center px-4 py-10 sm:px-6 lg:px-8">
      <motion.main initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 shadow-2xl backdrop-blur-3xl light-mode:border-black/10 light-mode:bg-white/80">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-tr from-indigo-500 to-emerald-400 shadow-lg shadow-indigo-500/30">
          ...
          <h1 className="text-4xl font-bold text-white light-mode:text-slate-900">GeoShield-AI</h1>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/30 px-5 py-3.5 text-white outline-none transition focus:border-indigo-400 light-mode:border-black/10 light-mode:bg-white light-mode:text-slate-900" />
          </div>
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/30 px-5 py-3.5 text-white outline-none transition focus:border-indigo-400 light-mode:border-black/10 light-mode:bg-white light-mode:text-slate-900" />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full rounded-2xl bg-gradient-to-r from-indigo-500 to-sky-500 py-4 font-bold text-white shadow-lg shadow-indigo-500/20 disabled:opacity-50">{loading ? "Authenticating..." : "Sign In"}</button>
        </form>
